...
输入稳定性场景在连续键入、原生中文输入法组合输入和窗口缩放时持续发送相同 SSE 快照，检查无效滚动写入、页面与输入框是否被重建、焦点是否丢失、尺寸监听错误，以及浏览器录制帧中的异常暗帧。桌面和手机视口均运行；该检查不能替代特定设备显卡、驱动和浏览器扩展环境的验证。

输入焦点验收覆盖侧栏移除后立即进入输入框、延迟焦点恢复、键盘关闭后回到入口，以及模型菜单和输入法组合输入不会误触发 `n` 新任务快捷键。它使用受控的关闭时序复现竞争，不依赖随机点击速度。

删除场景覆盖连续退格、跨行删除、全选删除、中文组合输入缩短与取消、删空时发送/停止按钮切换，以及回复持续增长时删除草稿和指令标签。自动化检查未覆盖用户设备的实际显卡合成结果；本地通过不能单独证明某台设备的黑屏现象已消失。

手机视口验收 `tests/browser_mobile_viewport.py` 已纳入 smoke，模拟双指缩放、软键盘高度和偏移变化，并在输入、删除期间注入重复及小数像素事件。它检查缩放不会收缩工作台、尺寸噪声不会反复改写根节点、实际键盘尺寸变化仍能适配；模拟事件不能替代真机软键盘验证。

`tests/browser_dialog_keyboard.py` 反复开关新任务和嵌套画板、模拟键盘收放，并在弹窗退出时立即重新聚焦输入框，检查残留的关闭遮罩、焦点被抢走、页面失去交互和输入删除异常。另有持续逐帧改变键盘视口时快速关闭、重开弹窗及输入删除的检查；验收使用合成会话，不提交真实任务。

问答验收 `tests/browser_user_inputs.py` 覆盖桌面/手机的选项、自由填写、草稿同步、长回复下的可见性与倒计时，已纳入 smoke。`tests/test_user_inputs.py` 验证真实异步工具记录的解析、答案标识、30 秒到期、重复提交竞争、重启后的去重和失败重试。运行 `.venv/bin/python tests/input_smoke.py` 可用隔离服务分别验证外部队列与网页托管任务的 JSON-RPC 回传：每种场景实际等待 30 秒，检查手动/自动投递各一次，托管任务的答案进入当前回复的原生记录；不访问真实会话或调用模型。
...
回复定位验收覆盖短回复增长为长回复、执行结束、手动滚动、窗口缩放、历史加载、会话切换和延迟发送回执，检查实际消息位置是否保持在回答开头或用户选择的阅读位置。已纳入 smoke，也可运行 `tests/browser_reply_scroll.py --url 地址 --output runtime/scroll-check`；全部 API 和 SSE 使用合成数据。
